...
app = Flask(__name__)
app.secret_key = "pixel_ultra_final_v8_search_added"
...
def home():
    ...
    if q:
        prods = Product.query.filter(Product.name.contains(q)).all()
    else:
        prods = Product.query.all()
    
    search_html = f"""
    <form action="/" method="GET" class="search-container">
        <input name="q" class="search-input" placeholder="Search products..." value="{q if q else ''}">
    ...
    final_content = search_html + (grid_html if prods else "<p>No products found.</p>")
    return wrap(final_content)
...
def cart():
    ...
    checkout_form = "<form action='/checkout' method='post'><textarea name='addr' placeholder='Delivery Address' required></textarea><button class='btn btn-accent' style='width:100%'>PLACE ORDER</button></form>" if items else ""
    content = f"{status_section}<div class='card'><h2 class='pixel'>MY BAG</h2>{bag_items if items else '<p>Your bag is empty.</p>'}<hr><h3 style='text-align:right;'>Total: {total} BDT</h3>{checkout_form}</div>"
    return wrap(content)
...
def admin():
    ...
    for o in ords:
        admin_ui += f"<div class='card' style='font-size:13px;'><b>#{o.id}</b> | {o.user_phone} | <b class='st-{o.status}'>{o.status}</b><br><div style='margin-top:8px;'><a href='/st/{o.id}/Packaging' class='btn btn-blue'>PACK</a><a href='/st/{o.id}/Delivered' class='btn btn-accent'>DONE</a><a href='/st/{o.id}/Cancelled' class='btn btn-main'>X</a><a href='/del_o/{o.id}' class='btn btn-dark' onclick='return confirm(\"Delete Order?\")'>DEL</a></div></div>"
