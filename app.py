...
<style>
    ...
    .btn { padding: 10px; border: 2px solid #000; cursor: pointer; text-decoration: none; display: inline-block; font-family: 'Press Start 2P'; font-size: 7px; color: white; text-align: center; }
    ...
    .status-Pending { color: orange; } .status-Packaging { color: blue; } .status-Delivered { color: green; } .status-Cancelled { color: red; }
</style>
"""
...
def wrap(content):
    ...
    <div class="header"><div class="pixel">PIXELULTRA</div><div class="pixel">{'ADMIN' if session.get('is_admin') else ''}</div></div>
...
def home():
    ...
    for p in products:
        html += f"""<div class="card" style="text-align:center;">
            <a href="/product/{p.id}"><img src="{p.image}" style="width:100%; height:120px; object-fit:contain;"></a>
            <p class="pixel" style="font-size:7px;">{p.name}</p>
    ...
    html += '</div>'
    return wrap(html if products else "<p>No items in shop!</p>")

@app.route('/product/<int:id>')
def details(id):
    p = Product.query.get(id)
    return wrap(f"""<div class="card" style="text-align:center;">
        <img src="{p.image}" style="width:100%; border-bottom:2px solid #000;">
        <h2 class="pixel">{p.name}</h2>
        <p class="pixel" style="color:blue;">{p.stock}</p>
        <p style="font-size:22px;">{p.desc}</p>
        <h2 style="color:var(--main);">{p.price} BDT</h2>
        <a href="/add_to_cart/{p.id}" class="btn btn-accent" style="width:100%; box-sizing:border-box;">ADD TO BAG</a>
    </div>""")

@app.route('/login', methods=['GET', 'POST'])
def login():
    if 'user_id' in session:
        return wrap(f"<div class='card'><h3 class='pixel'>Welcome, {session.get('u_name')}</h3><p>Phone: {session.get('u_phone')}</p><a href='/logout' class='btn btn-main'>LOGOUT</a></div>")
...
def register():
    if request.method == 'POST':
        ...
        db.session.add(User(phone=request.form['phone'], name=request.form['name'], password=pwd))
        db.session.commit(); return redirect('/login')
    return wrap("<div class='card'><h2 class='pixel'>REGISTER</h2><form method='post'><input name='name' placeholder='Full Name'><input name='phone' placeholder='Mobile Number'><input name='password' type='password' placeholder='Create Password'><button class='btn btn-accent' style='width:100%'>SIGN UP</button></form></div>")
...
def cart():
    ...
    # Order Status section inside Bag for Customer
    status_html = ""
    if 'user_id' in session:
        my_orders = Order.query.filter_by(user_id=session['user_id']).order_by(Order.id.desc()).all()
        if my_orders:
            status_html = "<div class='card' style='background:#eee;'><h3 class='pixel'>ORDER STATUS</h3>"
            for o in my_orders:
                status_html += f"<p style='font-size:14px;'>Order #{o.id}: <b class='status-{o.status}'>{o.status}</b></p>"
            status_html += "</div>"
    ...
    <div class='card'>
        ...
        { "<form action='/checkout' method='post'><textarea name='addr' placeholder='Full Delivery Address' required></textarea><button class='btn btn-accent' style='width:100%'>PLACE ORDER</button></form>" if items else "" }
    </div>""")
...
def admin():
    ...
    html = """<div class='card'><h3 class='pixel'>ADD ITEM</h3><form method='post' enctype='multipart/form-data'>
    <input name='name' placeholder='Name'><input name='price' placeholder='Price'>
    <input type='file' name='img_file'><input name='img_url' placeholder='OR Img URL'>
    <textarea name='desc' placeholder='Desc'></textarea>
    <select name='stock'><option>In Stock</option><option>Out of Stock</option></select>
    <button name='add_p' class='btn btn-dark' style='width:100%'>SAVE</button></form></div>"""
    
    html += "<h3 class='pixel'>ORDERS</h3>"
    for o in ords:
        html += f"<div class='card' style='font-size:12px;'>#{o.id} | {o.status}<br><a href='/st/{o.id}/Packaging' class='btn btn-dark'>PACK</a> <a href='/st/{o.id}/Delivered' class='btn btn-accent'>SHIP</a> <a href='/del_p/{o.id}' style='color:red;'>DEL</a></div>"
    return wrap(html)
