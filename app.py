...
app = Flask(__name__)
app.secret_key = "pixel_ultra_secret_777"

# --- GMAIL CONFIG (Apnar dewa password ekhane boshanu hoyeche) ---
SENDER_EMAIL = "apnar-email@gmail.com" # Ekhane oboshoy apnar Gmail-ti likhun
...
# --- HTML TEMPLATE (All-in-one with Pixel Style) ---
BASE_HTML = """
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div style="display:flex; justify-content:space-between; margin-bottom:20px;">
        <div>
            {% if session.get('user') %}
                <span>Hi, {{ session['user'] }}</span> | <a href="/logout" class="btn" style="background:gray">LOGOUT</a>
            {% else %}
...
def send_otp():
    ...
    try:
        ...
        msg = f"Subject: PixelShop Login OTP\\n\\nYour code is: {otp}"
        ...
        {% endblock %}
        """)
    except: return "Email Error! Check App Password."
...
def cart():
    ...
    <div class="box">
        ...
        <form action="/checkout" method="post">
            ...
            <button type="submit" class="btn" style="background:green">ORDER NOW</button>
        </form>
    </div>
    {% endblock %}
    """, items=items)
...
def admin_login():
    if request.method == 'POST':
        if request.form['u'] == ADMIN_U and request.form['p'] == ADMIN_P:
            ...
            return redirect('/')
    return render_template_string(BASE_HTML + """
    {% block content %}<form method="post"><input name="u" placeholder="User"><input name="p" type="password" placeholder="Pass"><button class="btn">LOGIN</button></form>{% endblock %}
    """)
